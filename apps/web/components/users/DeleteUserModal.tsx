...
export default function DeleteUserModal({ onClose, user }: Props) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      try {
        ...
        console.error(error);
        setToastMessage({ type: 'error', text: "❌ Failed to delete user." });
      }
    });
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-8 shadow-2xl">
        ...
        <p className="mb-8 text-slate-300">
          Are you sure you want to delete <strong className="text-white">{user.firstName} {user.lastName}</strong>? This will permanently remove their access and data. This action cannot be undone.
        </p>
